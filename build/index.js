...
var ASM_CONSTS = {
  68096: ($0) => { try { return FS.analyzePath(UTF8ToString($0)).exists; } catch(e){ return 0; } },  
 68178: ($0) => { try { return FS.analyzePath(UTF8ToString($0)).object.isFolder; } catch(e){ return 0; } },  
 68269: ($0) => { try { FS.mkdir(UTF8ToString($0)); } catch(e){ console.log(e); } },  
 68337: ($0) => { try { FS.rmdir(UTF8ToString($0)); } catch(e){ console.log(e); } },  
 68405: ($0) => { try { FS.unlink(UTF8ToString($0)); } catch(e){ console.log(e); } },  
 68474: ($0, $1) => { var x = $0; var y = $1; if(document.pointerLockElement === Module['canvas']){ } else { } },  
 68567: () => { var canvas = Module['canvas']; canvas.style.cursor = 'none'; },  
 68632: () => { var canvas = Module['canvas']; canvas.style.cursor = 'default'; },  
 68700: ($0, $1, $2) => { var width = $0; var height = $1; var ptr = $2; var canvas = Module['canvas']; if(canvas.width !== width) canvas.width = width; if(canvas.height !== height) canvas.height = height; var ctx = canvas.getContext('2d'); var imageData = ctx.createImageData(width, height); var src = HEAPU8.subarray(ptr, ptr + width * height * 4); imageData.data.set(src); ctx.putImageData(imageData, 0, 0); },  
 69089: ($0) => { if (navigator.clipboard) { var text = UTF8ToString($0); navigator.clipboard.writeText(text); } else { console.log("Clipboard API not available"); } }
};
